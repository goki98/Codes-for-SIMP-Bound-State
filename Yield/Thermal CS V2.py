...
if __name__ == "__main__":
  def CS(v_rel_km_s):
    ...
    P11 = np.sqrt((float(s11) + 2.0*mx)**2 - 4.0*float(s11)*(2.0*mx)**2) / float(s11)
    ...
    return cs3,CS1_4,CS2_4,CS3_4,CS1_9,CS2_9,CS3_9,CS11_10,CS12_10,CS13_10,CS23_10,CS22_10,CS33_10,CS11_11,CS12_11,CS13_11,CS23_11,CS22_11,CS33_11,CS111_12,CS112_12,CS113_12,CS122_12,CS123_12,CS133_12,CS222_12,CS223_12,CS233_12,CS333_12
    # -------------------------
   # Calculate thermal average for a specific cross section index
def thermal_avg_mc(n_samples, x):
    ...
    sigma = np.sqrt(2/x)  # width parameter
    v_samples = np.random.normal(0, sigma, size=(n_samples, 3))  # 3D velocities
    v_magnitudes = np.linalg.norm(v_samples, axis=1)

    results = np.zeros(29)  # CS returns 29 values
    for v in v_magnitudes:
        cs_vals = CS(v)  # tuple of 29 values
        results += np.array(cs_vals)

    results /= n_samples  # average over samples
    return results


# Run MC integration
ThermCS_results = thermal_avg_mc(n_samples=50, x=3)

# Labels for unpacking
labels = [
    "cs3", "CS1_4","CS2_4","CS3_4","CS1_9","CS2_9","CS3_9",
    "CS11_10","CS12_10","CS13_10","CS23_10","CS22_10","CS33_10",
...
# Print all results
for name, val in zip(labels, ThermCS_results):
    print(f"{name}: {val:.6e}")

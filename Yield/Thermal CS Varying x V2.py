...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 26 15:09:29 2025
...
if __name__ == "__main__":
  def CS(v_rel_km_s):
    ...
    beta_rel=v_rel_km_s #c=1 in natural unit, beta= vrel/c
    s=4*4*mx**2
    V0 = g_x**2
    mu = mx / 2
    a0=1/(mu*V0)
    ...
    CS2_4=(MatSq2_4*p2_4)/(64*np.pi*mx**4)
    ...
    cs4_t=CS1_4+CS2_4+CS3_4
    cs9_t=CS1_9+CS2_9+CS3_9
    cs10_t=CS11_10+CS12_10+CS13_10+CS23_10+CS22_10+CS33_10
    cs11_t=CS11_11+CS12_11+CS13_11+CS23_11+CS22_11+CS33_11
    cs12_t=CS111_12+CS112_12+CS113_12+CS122_12+CS123_12+CS133_12+CS222_12+CS223_12+CS233_12+CS333_12
    return cs12_t,cs11_t,cs3,cs4_t,CS1_4,CS2_4,CS3_4,cs9_t,CS1_9,CS2_9,CS3_9,cs10_t,CS11_10,CS12_10,CS13_10,CS23_10,CS22_10,CS33_10,CS11_11,CS12_11,CS13_11,CS23_11,CS22_11,CS33_11,CS111_12,CS112_12,CS113_12,CS122_12,CS123_12,CS133_12,CS222_12,CS223_12,CS233_12,CS333_12
    # -------------------------
def thermal_avg_mc(n_samples, x):
    """
    Monte Carlo estimate of thermal averaged cross sections.
    """
    sigma = np.sqrt(2/x)  # width parameter for Maxwell-Boltzmann
    v_samples = np.random.normal(0, sigma, size=(n_samples, 3))
    v_magnitudes = np.linalg.norm(v_samples, axis=1)

    # Get the size of the CS output
    first_cs = CS(v_magnitudes[0])
    n_cs = len(first_cs)
    results = np.zeros(n_cs)

    # Monte Carlo averaging
    for v in v_magnitudes:
        cs_vals = CS(v)
        results += np.array(cs_vals)
...
# Sampling parameters
x_values = np.linspace(0.5, 10000,20 )
n_samples = 100  # increase for smoother averaging

# Call once to get number of outputs
test_cs = CS(0.1)
n_outputs = len(test_cs)

# Allocate storage
ThermCS_all = np.zeros((len(x_values), n_outputs))

# Compute thermal averages for each x
for i, x in enumerate(x_values):
    ThermCS_all[i] = thermal_avg_mc(n_samples=n_samples, x=x)

# Define the exact variable names in the same order as returned by CS()
var_names = [
    "cs12_t","cs11_t","cs3","cs4_t",
    "CS1_4","CS2_4","CS3_4",
    "cs9_t","CS1_9","CS2_9","CS3_9",
    "cs10_t","CS11_10","CS12_10","CS13_10","CS23_10","CS22_10","CS33_10",
    "CS11_11","CS12_11","CS13_11","CS23_11","CS22_11","CS33_11",
    "CS111_12","CS112_12","CS113_12","CS122_12","CS123_12","CS133_12",
    "CS222_12","CS223_12","CS233_12","CS333_12"
]

# Include x as first column
col_labels = ["x"] + var_names

# Save to CSV
df = pd.DataFrame(np.column_stack([x_values, ThermCS_all]), columns=col_labels)
df.to_csv("thermal_avg_all_CS2.csv", index=False)

print(f"✅ Thermal averages saved with correct variable names ({len(var_names)} columns).")

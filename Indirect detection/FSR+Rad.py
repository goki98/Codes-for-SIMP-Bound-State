...
prefact_flux=0.5*(r_sun/(4*np.pi))*(d_sun/mx)**2 * J_factor
# FSR spectrum calc
# Energy range setup
...
if Q > 2 * M_ele:
    spectrum = dN_dEgamma_lep(Egamma, Q, M_ele)
    Diff_flux_e_C1 = prefact_flux * ThermCS_Ce1 * spectrum
    Diff_flux_e_C2 = prefact_flux * ThermCS_Ce2 * spectrum
    Diff_flux_e_C3 = prefact_flux * ThermCS_Ce3* spectrum
...
if Q > 2 * M_mu:
    spectrum = dN_dEgamma_lep(Egamma, Q, M_mu)
    Diff_flux_m_C1 = prefact_flux * ThermCS_Cm1 * spectrum
    Diff_flux_m_C2 = prefact_flux * ThermCS_Cm2 * spectrum
    Diff_flux_m_C3 = prefact_flux * ThermCS_Cm3 * spectrum
...
if Q > 2 * mpiC:
    spectrum_pion = dN_dEgamma_pion(Egamma, Q, mpiC)
    Diff_flux_pc_C1 = prefact_flux * ThermCS_Cpc1 * spectrum_pion
    Diff_flux_pc_C2 = prefact_flux * ThermCS_Cpc2 * spectrum_pion
    Diff_flux_pc_C3 = prefact_flux * ThermCS_Cpc3 * spectrum_pion
...
Diff_flux_Dec_M_Rad_1= prefact_flux * ThermCS_Cm1  * spec_mu_rad
Diff_flux_Dec_M_Rad_2= prefact_flux * ThermCS_Cm2  * spec_mu_rad
Diff_flux_Dec_M_Rad_3= prefact_flux * ThermCS_Cm3  * spec_mu_rad

# pion radtive flux
# electron
Diff_flux_Ann_E_Rad=  prefact_flux * ThermCS_Cpc  * spec_e
Diff_flux_Dec_E_Rad_1= prefact_flux * ThermCS_Cpc1  * spec_e
Diff_flux_Dec_E_Rad_2= prefact_flux * ThermCS_Cpc2  * spec_e
Diff_flux_Dec_E_Rad_3= prefact_flux * ThermCS_Cpc3  * spec_e
# muon 
Diff_flux_Ann_mp_Rad=  prefact_flux * ThermCS_Cpc * spec_mu_direct
Diff_flux_Dec_mp_Rad_1= prefact_flux * ThermCS_Cpc1  * spec_mu_direct
Diff_flux_Dec_mp_Rad_2= prefact_flux * ThermCS_Cpc2  * spec_mu_direct
Diff_flux_Dec_mp_Rad_3= prefact_flux * ThermCS_Cpc3  * spec_mu_direct
#total
Diff_flux_Ann_tot=  prefact_flux * ThermCS_Cpc  * total_pion
Diff_flux_Dec_tot_1= prefact_flux * ThermCS_Cpc1  * total_pion
Diff_flux_Dec_tot_2= prefact_flux * ThermCS_Cpc2  * total_pion
Diff_flux_Dec_tot_3= prefact_flux * ThermCS_Cpc3  * total_pion
...
if Q > 2 * M_ele:
    ...
    plt.plot(Egamma, Egamma**2 * Elect_Tot_Dec, 'r--', label=rf"$e^\pm$")
else:
    print(f"Skipping e+e-: Q={Q:.1f} MeV < {2*M_ele:.1f} MeV")
...
if Q > 2 * M_mu:
    ...
    plt.plot(Egamma, Egamma**2 * Muon_tot_dec,'g--', label=rf"$\mu^\pm$")
else:
    print(f"Skipping μ+μ−: Q={Q:.1f} MeV < {2*M_mu:.1f} MeV")
...
if Q > 2 * mpiC:
    ...
    plt.plot(Egamma, Egamma**2 *pion_tot_dec,'b--' ,label=rf"$\pi^\pm$")
else:
    print(f"Skipping π+π−: Q={Q:.1f} MeV < {2*mpiC:.1f} MeV")
plt.xlabel(r"$E_\gamma$")
plt.ylabel(r"$E^2_\gamma \, d\Phi/dE_\gamma$")
plt.xscale('log')
plt.yscale('log')
labelLines(plt.gca().get_lines(), align=True,xvals=[1e0, 1e-1, 1e0])
...
if Q > 2 * M_ele:
    ...
    plt.plot(Egamma, Egamma**2 * Elect_Tot_ann, 'r--', label=rf"$e^\pm$")
else:
    print(f"Skipping e+e-: Q={Q:.1f} MeV < {2*M_ele:.1f} MeV")
...
if Q > 2 * M_mu:
    ...
    plt.plot(Egamma, Egamma**2 * Muon_tot_ann,'g--', label=rf"$\mu^\pm$")
else:
    print(f"Skipping μ+μ−: Q={Q:.1f} MeV < {2*M_mu:.1f} MeV")
...
if Q > 2 * mpiC:
    ...
    plt.plot(Egamma, Egamma**2 *pion_tot_ann,'b--' ,label=rf"$\pi^\pm$")
else:
    print(f"Skipping π+π−: Q={Q:.1f} MeV < {2*mpiC:.1f} MeV")
plt.xlabel(r"$E_\gamma$")
plt.ylabel(r"$E^2_\gamma \, d\Phi/dE_\gamma$")
...
labelLines(plt.gca().get_lines(), align=True,xvals=[1e0, 1e0, 1e0])
plt.grid(which="major", linestyle="--", linewidth=0.8, alpha=0.7)
plt.grid(which="minor", linestyle=":", linewidth=0.5, alpha=0.4)
plt.show()
